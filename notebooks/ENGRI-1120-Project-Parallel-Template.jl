...
md"""
### Introduction
What the Fugacity™  was tasked with designing and analyzing a process to produce isoprene from sucrose. Isoprene, also known as 2-methyl- 1,3 butadiene, is a high value organic molecule. Polymers of isoprene have industrial applications including rubber for tires and moldings. Isoprene’s empirical formula is $C_{5}$$H_{8}$ with a molecular weight of 68.12 grams/mol and density 0.681 grams/milliliter at room temperature.

Sucrose is a disaccharide sugar composed of glucose and fructose. It's molecular formula is $C_{12}$$H_{22}$$O_{11}$ and has molecular mass of 342.3 g/mol. Sucrose is a viable sugar for this process because it is cheap and abundant. 

What the Fugacity Labs will be producing isoprene in 100 microliter reactors using an E. coli cell free system.
...
# ╔═╡ 6eeb341a-e8b1-4321-bc25-0d419b64dcc8
md"""
In order to produce our product, What the Fugacity relied on methods of guess and check with the Flux Balance Analysis to reach purity and flow rate standards. Our rationale was to maximize the flow rate of isoprene while minimizing the input flow rates of expensive chemicals. We tested our system in series as well as parallel to understand which system would be most profitable. In order to optimize the output of Isoprene. A Flux Balance Analysis is a modeling approach (typically a linear programming problem) used to determine the flux of carbon and energy through metabolic networks.

We sought to produce >99% pure isoprene at as quick of a rate as possible with few separation steps, as our sale price could be significantly higher and we could minimize expenses.  
"""
...
# ╔═╡ 64313574-c44f-4151-a9a6-ef457c7c7c37
md"""
In a parallel configuration for chemical reactors, a feed from a syringe pump enters a splitter that separates the feed into 1000 equal streams. These streams are then fed into 1000 reactors (well-mixed continuous microfluidic chips). Each chip acts as an independent unit and all the chips produce the same output. The output from all the chips will be recombined and then enter the downstream separation unit. 

To separate isoprene from sucrose and various other byproducts, we used MSU’s (Magical Separating Units) purchased from Varnerlab. One stream enters the MSU and two streams exit, and the unit has an efficiency of 75%. 
...
begin
	...
	plot!(stages, target_line, color="red", lw=2,linestyle=:dash, label="VarnerLab Target 95% Purity")

	# make a 0.995 line target line -
	goal_line = 0.995*ones(number_of_levels)
	plot!(stages, goal_line, color="magenta", lw=2,linestyle=:dash, label="Analytical Standard")
end
...
Net present value (NPV) can be used to evaluate this project's value as compared to alternative investments. NPV is the net cash flow for each time period discounted back to the present value:

$$NPV = CF_{1}+\sum_{i=2}^{T}\left(\frac{1}{d_{i1}}\right)CF_{i}$$

Where $CF_{i}$ is the net cash flow in year i and the discount factor term $d_{i,1}$ is given by:
...
Over the project lifetime, the Net Present Value calcuations show the strength of investing in What the Fugacity Isoprene production. In a 10 year run of WTF Systems (T= 10), where we pay off all fixed capital costs in the first year of production, the NPV presents as follows:
"""
...
md"""
### Conclusions
Because our NPV is positive when compared to both the minimum and maximum alternative investment yields, the production of isoprene from sucrose is a more viable investment than the alternative. Sucrose is relatively inexpensive and thus alternative feedstocks do not need to be explored. This process could be further scaled by increasing the number of chips to increase the production of isoprene depending on how much capital the investor is willing to commit. Several assumptions were made in this analysis which render application in a real world setting more complicated and potentially expensive than it appears.  
"""
...
Isoprene Background Information
<https://pubchem.ncbi.nlm.nih.gov/compound/isoprene>

Sucrose Background Information
<https://pubchem.ncbi.nlm.nih.gov/compound/sucrose>

Flux Balance Analysis Notes
<https://drive.google.com/drive/folders/1zSWDASS6ZNSS271n3nuSPtV-gjI88K6n>

Cost of Isoprene (Analytical Standard Reference Number 59240)
<https://www.sigmaaldrich.com/US/en/search/isoprene?focus=products&page=1&perPage=30&sort=relevance&term=isoprene&type=product>

Cost of Sucrose (Sucrose For Microbiology Reference Number S0389) 
<https://www.sigmaaldrich.com/US/en/search/sucrose?focus=products&page=1&perPage=30&sort=relevance&term=sucrose&type=product_name> 
